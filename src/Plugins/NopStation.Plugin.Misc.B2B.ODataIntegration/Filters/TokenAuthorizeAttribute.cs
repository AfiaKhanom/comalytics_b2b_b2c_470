...
using NopStation.Plugin.Misc.B2B.ODataIntegration.Enums;
using NopStation.Plugin.Misc.B2B.ODataIntegration.Extensions;
using NopStation.Plugin.Misc.B2B.ODataIntegration.Model.Auth;

namespace NopStation.Plugin.Misc.B2B.ODataIntegration.Filters;
        ...
        )
        {
            if (
                actionContext.HttpContext.Request.Headers.TryGetValue(
                    D365IntegrationDefaults.Token,
                    out var checkToken
                )
            )
            {
                var sysproIntegrationSettings =
                    EngineContext.Current.Resolve<D365IntegrationSettings>();
                var token = checkToken.FirstOrDefault();
                if (!string.IsNullOrEmpty(token) && token.StartsWith("Bearer "))
                    token = token.Substring("Bearer ".Length);
                var secretKey = sysproIntegrationSettings.IntegrationSecretKey;
                try
                {
                    var payload =
