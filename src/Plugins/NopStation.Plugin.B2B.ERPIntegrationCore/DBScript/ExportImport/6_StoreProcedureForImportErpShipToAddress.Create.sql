/****** Object:  StoredProcedure [dbo].[ErpShipToAddressImportProcedure]    Script Date: 8/29/2025 1:20:14 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			DECLARE @currentShipToAddressId INT =
			(
				SELECT TOP(1) s.Id
				FROM [dbo].[Erp_ShipToAddress] s WITH (NOLOCK)
				INNER JOIN [dbo].[Erp_ShiptoAddress_Erp_Account_Map] m WITH (NOLOCK)
					ON m.ErpShiptoAddressId = s.Id
				WHERE s.ShipToCode = @ShipToCode
					AND m.ErpShipToAddressCreatedByTypeId = 10
			);
