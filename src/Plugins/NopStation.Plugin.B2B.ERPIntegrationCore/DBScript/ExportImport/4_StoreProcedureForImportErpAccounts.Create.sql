/****** Object:  StoredProcedure [dbo].[ErpAccountImportProcedure]    Script Date: 11/24/2025 11:29:58 AM ******/
SET ANSI_NULLS ON
GO
...
DECLARE @AccountNumber nvarchar(50),
	...
	@CreditLimitAvailable nvarchar (100),
	...
	@OverrideStockDisplayFormatConfigSetting nvarchar (10),
	...
	FETCH NEXT FROM account_import_cursor INTO @AccountNumber, @AccountName, @SalesOrganisationCode, 
			...
			@VatNumber, @CreditLimit, @CreditLimitAvailable, @CurrentBalance, @AllowOverspend, 
			@PriceGroupCode, @PreFilterFacets, @PaymentTypeCode, @OverrideBackOrderingConfigSetting, @AllowAccountsBackOrdering, @OverrideStockDisplayFormatConfigSetting,
		...
		BEGIN
			...
			INSERT INTO [dbo].[Address]
				...
				VALUES
					...
					,@BillingEmail
					,@BillingCompany
					,CASE WHEN @CountryId = 0 THEN NULL ELSE @CountryId END
					,CASE WHEN @StateProvinceId = 0 THEN NULL ELSE @StateProvinceId END
					,@BillingCity
					,@BillingAddress1
					,@BillingAddress2
					,@BillingZipPostalCode
			...
			VALUES
				...
				,CAST(@CreditLimitAvailable AS DECIMAL(18, 4))
				...
				END
				,CASE
					When @OverrideStockDisplayFormatConfigSetting = 'True' OR @OverrideStockDisplayFormatConfigSetting = 'true' OR @OverrideStockDisplayFormatConfigSetting = 'TRUE' THEN 1
					ELSE 0
				END
				,CASE
			...
			BEGIN
				...
				BEGIN
					...
					SET [FirstName] = @BillingFirstName
						...
						,[StateProvinceId] = CASE WHEN @StateProvinceId = 0 THEN NULL ELSE @StateProvinceId END
						,[City] = @BillingCity
				...
				UPDATE [dbo].[Erp_Account]
					SET [IsActive] = 
						...
						,[CreditLimitAvailable] = CAST(@CreditLimitAvailable AS DECIMAL(18, 4))
