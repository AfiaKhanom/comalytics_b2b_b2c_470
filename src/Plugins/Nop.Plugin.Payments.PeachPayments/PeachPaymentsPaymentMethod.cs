...
public class PeachPaymentsPaymentMethod : BasePlugin, IPaymentMethod
{
    ...
    private readonly PeachPaymentsSettings _peachPaymentSettings;
        ...
        ILogger logger)
    {
        ...
        _peachPaymentSettings = settings;
    ...
    {
        var method_description = _peachPaymentSettings.PeachPaymentsCheckoutDisplayText;
    ...
    {
        if (string.IsNullOrWhiteSpace(_peachPaymentSettings.ApplicableForRoleIds))
        {
            return true;
        }

        var validRoleIds = _peachPaymentSettings.ApplicableForRoleIds.Split(',').Select(int.Parse).ToList();

        var customer = await _workContext.GetCurrentCustomerAsync();

        var currentCustomerRoleIds = (await _customerService.GetCustomerRolesAsync(customer)).Select(x => x.Id).ToList();
    ...
    public async Task PostProcessPaymentAsync(PostProcessPaymentRequest postProcessPaymentRequest)
    {
        ...
        var isSandboxMode = _peachPaymentSettings.SandBoxModeId == (int)SandboxMode.Enabled;
        var checkoutUrl = isSandboxMode ? "https://testsecure.peachpayments.com/checkout/initiate" : "https://secure.peachpayments.com/checkout/initiate";
        var ourBaseUrl = _webHelper.GetStoreLocation();
        ...
        var peachcurrencycode = await _currencyService.GetCurrencyByIdAsync(_peachPaymentSettings.CurrencyId);
        ...
        if (isSandboxMode)
        {
            remotePostHelperData.Add("authentication.entityId", _peachPaymentSettings.CheckoutChannelSandbox);
        }
        else
        {
            remotePostHelperData.Add("authentication.entityId", _peachPaymentSettings.CheckoutChannel);
        }
        ...
        remotePostHelperData.Add("merchantTransactionId", _peachPaymentSettings.MerchantIdPrefix + postProcessPaymentRequest.Order.Id.ToString());
        ...
        var signature = GenerateSignature(remotePostHelperData, isSandboxMode);

        remotePostHelperData.Add("signature", signature);

        try
        {
            var content = JsonContent.Create(remotePostHelperData);
            ...
            {
                var value = await response.Content.ReadAsStringAsync();
                await _logger.InsertLogAsync(LogLevel.Information, $"Error while redirecting to Peach Payments for order id: {postProcessPaymentRequest.Order.Id}, Error: {value}");
    ...
    public string GenerateSignature(Dictionary<string, string> remotePostHelperData, bool isSandbox)
    {
        ...
        if (isSandbox)
            keyhmac = _peachPaymentSettings.SecretTokenSandbox;
        else
            keyhmac = _peachPaymentSettings.SecretToken;
        var hash = new HMACSHA256(enc.GetBytes(keyhmac));
        var baHashedText = hash.ComputeHash(enc.GetBytes(signaturedata));
    ...
    public override async Task InstallAsync()
    {
        ...
        await _localizationService.AddOrUpdateLocaleResourceAsync(new Dictionary<string, string>
        {
            ...
            ["Plugins.Payments.PeachPayments.Fields.ApplicableForRoleIds"] = "Applicable For Customer Roles",
        });
