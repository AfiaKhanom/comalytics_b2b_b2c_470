...
using Nop.Plugin.Payments.PeachPayments.PeachPayments.Checkout;
    ...
    private async Task<(TResult Result, string Error)> HandleFunctionAsync<TResult>(Func<Task<TResult>> function)
    {
        ...
        catch (Exception exception)
        {
            ...
            if (exception is HttpException httpException)
            {
                ...
                {
                    message += details.Details.Aggregate(":", (text, issue) => 
                        $"{text} " + 
            ...
            //log errors
            var logMessage = $"{PeachPaymentsDefaults.SystemName} error: {Environment.NewLine}{message}";
