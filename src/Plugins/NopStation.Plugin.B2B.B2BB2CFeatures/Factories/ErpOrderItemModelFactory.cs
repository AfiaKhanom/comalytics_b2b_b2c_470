...
public class ErpOrderItemModelFactory : IErpOrderItemModelFactory
{
    ...
    public async Task<ErpOrderDetailsModel> PrepareB2BOrderItemDataModelListModelAsync(int nopOrderId, List<string> itemIds)
    {
        ...
        var b2bOrder = 
            erpOrder.ErpOrderType == ErpOrderType.B2BSalesOrder || erpOrder.ErpOrderType == ErpOrderType.B2BQuote ? erpOrder : null;
        var b2COrder = 
            erpOrder.ErpOrderType == ErpOrderType.B2CSalesOrder || erpOrder.ErpOrderType == ErpOrderType.B2CQuote ? erpOrder : null;
    ...
    public async Task<ErpCheckoutCompletedModel> PrepareB2BCheckoutCompletedModelAsync(int nopOrderId)
    {
        ...
        if (b2BAccount != null && (b2bOrder != null || b2COrder != null))
        {
            ...
            {
                if (b2BAccount.AllowOverspend && b2BAccount.CreditLimit < b2BAccount.CurrentBalance && _b2BB2CFeaturesSettings.IsShowOverSpendWarningText)
                {
            ...
            if (!b2BCheckoutCompleted.IsErpIntegrationSuccess)
            {
                ...
                b2BCheckoutCompleted.IntegrationError = 
                    await _localizationService.GetLocalizedEnumAsync(error ?? ErpOrderType.B2CSalesOrder) + b2bOrder?.IntegrationError ?? b2COrder?.IntegrationError;
            }
