...
public class B2BB2CWebWorkContext : IB2BB2CWorkContext
{
    #region Fields
    ...
    public B2BB2CWebWorkContext(CookieSettings cookieSettings,
    ...
    #region B2B/B2C Utilities

    protected async Task<ERPCustomer> PrepareERPCustomerAsync(Customer customer, Customer originalCustomerIfImpersonated, int erpAccountId = 0)
    {
        ...
        if (erpNopUser != null)
        {
            ...
            erpCustomer.CustomerRolesIds = (await _erpNopUserAccountMapService.GetErpNopUserAccountMapByAccountAndUserIdAsync(erpNopUser.ErpAccountId, erpNopUser.Id))?.CustomerRolesIds;
        }
    ...
    #region B2B/B2C properties

    public virtual async Task<ERPCustomer> GetCurrentERPCustomerAsync(int erpAccountId = 0)
    {
        //whether there is a cached value
        if (_cachedERPCustomer != null)
            return _cachedERPCustomer;

        await SetCurrentERPCustomerAsync(erpAccountId: erpAccountId);
    ...
    public virtual async Task SetCurrentERPCustomerAsync(Customer customer = null, int erpAccountId = 0)
    {
        ...
        if (!customer.Deleted && customer.Active && !customer.RequireReLogin)
        {
            ...
            //Prepare ERP Customer for B2B/B2C
            var erpCustomer = await PrepareERPCustomerAsync(customer, _originalCustomerIfImpersonated, erpAccountId);
            //cache the found customer
            _cachedCustomer = customer;
            _cachedERPCustomer = erpCustomer;
        }
    }
