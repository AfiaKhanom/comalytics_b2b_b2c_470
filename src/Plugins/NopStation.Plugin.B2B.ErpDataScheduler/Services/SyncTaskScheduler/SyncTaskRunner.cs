...
public partial class SyncTaskRunner : ISyncTaskRunner
{
    ...
    public async Task ExecuteAsync(SyncTask task, bool forceRun = false, bool throwException = false, bool ensureRunOncePerPeriod = true)
    {
        ...
        try
        {
            ...
            //await _locker.PerformActionWithLockAsync(task.Type, expiration, async () => await PerformTaskAsync(task));
            await PerformTaskAsync(task);
        }
        catch (Exception exc)
        {
            try
            {
                ...
                if (!task.LastStartUtc.HasValue)
                    task.LastStartUtc = DateTime.UtcNow.AddSeconds(-1);
                ...
                var message = string.Format("The \"{0}\" scheduled task failed with the \"{1}\" error. Task type: \"{2}\". Store name: \"{3}\". Task run address: \"{4}\".",
                    task?.Name ?? string.Empty, exc?.Message ?? string.Empty, task?.Type ?? string.Empty, store?.Name ?? string.Empty, syncTaskUrl ?? string.Empty);
