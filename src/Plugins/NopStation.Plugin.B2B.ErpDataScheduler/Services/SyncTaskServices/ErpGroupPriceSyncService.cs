    ...
    public virtual async Task<bool> IsErpGroupPriceSyncSuccessfulAsync(string? priceCode, string? stockCode, bool isManualTrigger = false, bool isIncrementalSync = true, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var propsToSkip = new HashSet<string>(
                _dataMappingSettings.GroupPricePropertiesToExclude?
            ...
            foreach (var salesOrg in allErpSalesOrgs)
            {
                ...
                while (true)
                {
                    var erpGetRequestModel = new ErpGetRequestModel
                    {
                        ...
                        LastChangedDate = isIncrementalSync ? salesOrg?.LastErpGroupPriceSyncTimeOnUtc : null,
                    ...
                    {
                        if (salesOrg.Code.Trim().ToLower() != erpGroupPrice.SalesOrgCode.Trim().ToLower())
                        {
                            totalNotSyncedSoFar++;
                            continue;
                        }
                        ...
                        if (!string.IsNullOrEmpty(erpGroupPrice.GroupPriceCode))
                        {
                            ...
                            if (!processedErpGroupPriceCodes.Contains(oldErpGroupPriceCode?.Code ?? string.Empty))
                            {
                                ...
                                {
                                    if (!propsToSkip.Contains(nameof(ErpPriceGroupPricingDataModel.GroupPriceCode)))
                                        oldErpGroupPriceCode.Code = erpGroupPrice.GroupPriceCode;
                            ...
                            if (await IsValidErpGroupPriceCode(oldErpGroupPriceCode))
                            {
                                var oldErpGroupPrice = await _erpGroupPriceService.GetErpGroupPriceByErpPriceGroupCodeAndProductId
                                    (salesOrgId: salesOrg?.Id ?? 0, productId: product.Id, priceGroupCodeId: oldErpGroupPriceCode.Id);
                                ...
                                if (oldErpGroupPrice == null)
                                {
                                    ...
                                    oldErpGroupPrice.ErpSalesOrgId = salesOrg.Code.Trim().ToLower() == erpGroupPrice.SalesOrgCode.Trim().ToLower() ? salesOrg.Id : 0;
                                ...
                                {
                                    if (!propsToSkip.Contains(nameof(ErpPriceGroupPricingDataModel.Price)))
                                        oldErpGroupPrice.Price = erpGroupPrice.Price ?? 0;
                            ...
                            foreach (var price in erpGroupPrice.GroupPrices)
                            {
                                if (price.Value > 0)
                                {
                                    ...
                                    if (!processedErpGroupPriceCodes.Contains(oldErpGroupPriceCode?.Code ?? string.Empty))
                                    {
                                        ...
                                        {
                                            if (!propsToSkip.Contains(nameof(ErpPriceGroupPricingDataModel.GroupPrices)))
                                                oldErpGroupPriceCode.Code = price.Name;
                                    ...
                                    if (await IsValidErpGroupPriceCode(oldErpGroupPriceCode))
                                    {
                                        ...
                                        .GetErpGroupPriceByErpPriceGroupCodeAndProductId(salesOrgId: salesOrg.Id, productId: product.Id, priceGroupCodeId: oldErpGroupPriceCode.Id);
                ...
                var isFullSync = string.IsNullOrWhiteSpace(priceCode) && string.IsNullOrWhiteSpace(stockCode);
                if (!isIncrementalSync && isFullSync)
                    await _erpGroupPriceService.DeleteGroupPricesNotUpdatedSinceSyncStart(salesOrg.Id, syncStartTime);
            }
