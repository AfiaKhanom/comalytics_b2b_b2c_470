...
public class ErpDataSchedulerPlugin : BasePlugin, IAdminMenuPlugin, IMiscPlugin, INopStationPlugin
{
    ...
    private readonly IScheduleTaskService _nopScheduleTaskService;
    ...
    private const string PLUGIN_NEXT_VERSION = "4.70.2.5";
    ...
    public ErpDataSchedulerPlugin(ILocalizationService localizationService,
        ...
        IScheduleTaskService nopScheduleTaskService)
    {
        _localizationService = localizationService;
        _webHelper = webHelper;
        _syncTaskService = taskService;
        _nopScheduleTaskService = nopScheduleTaskService;
    }
    ...
    public override async Task InstallAsync()
    {
        ...
        //install Erp Account Credit Sync task
        if ((await _syncTaskService.GetTaskByTypeAsync(ErpDataSchedulerDefaults.ErpAccountCreditSyncTask)) is null)
        {
            await _syncTaskService.InsertTaskAsync(new SyncTask
            {
                Enabled = false,
                LastEnabledUtc = DateTime.UtcNow,
                Name = ErpDataSchedulerDefaults.ErpAccountCreditSyncTaskName,
                Type = ErpDataSchedulerDefaults.ErpAccountCreditSyncTask
            });
        }

        //install Sync Log delete task
        if (await _nopScheduleTaskService.GetTaskByTypeAsync(ErpDataSchedulerDefaults.SyncLogFileDeleteTask) is null)
        {
            await _nopScheduleTaskService.InsertTaskAsync(new ScheduleTask
            {
                Enabled = true,
                LastEnabledUtc = DateTime.UtcNow,
    ...
    public List<KeyValuePair<string, string>> PluginResouces()
    {
        return new List<KeyValuePair<string, string>>
        {
            ...
            new ("Plugin.Misc.NopStation.ErpDataScheduler.Admin.Configure.Block.Configure", "Configure"),
            new ("Plugin.Misc.NopStation.ErpDataScheduler.Admin.Configure.Fields.SyncFromDate", "Sync from"),
            new ("Plugin.Misc.NopStation.ErpDataScheduler.Admin.Configure.Fields.SyncFromDate.Hint", "Select a date for the Syncs to start from"),
            new ("Plugin.Misc.NopStation.ErpDataScheduler.Admin.Configure.Fields.NeedQuoteOrderCall", "Need Quote Order Call"),
            new ("Plugin.Misc.NopStation.ErpDataScheduler.Admin.Configure.Fields.NeedQuoteOrderCall.Hint", "Check to enable Quote Order Call"),
            new ("Plugin.Misc.NopStation.ErpDataScheduler.Admin.Configure.Fields.StartProductSyncAfterLastSyncedProduct", "Start Product Sync after Last Synced Product"),
            new ("Plugin.Misc.NopStation.ErpDataScheduler.Admin.Configure.Fields.StartProductSyncAfterLastSyncedProduct.Hint", "Check to enable Product Sync to start right after the Last Synced Product"),
            ...
            new ("Plugin.Misc.NopStation.ErpDataScheduler.Tasks.IsRunning", "The schedule task is already running"),
            ...
            new ("Plugin.Misc.NopStation.ErpDataScheduler.Tasks.24days", "Erp Data Scheduler Task period should not exceed 24 days."),
